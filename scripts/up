...
major)
  ...
  _new_branch="$(($_major + 1)).0.0-branch"
  ;;
minor)
  _new_version="$_major.$(($_minor + 1)).0.0"
  _new_branch="$_major.$(($_minor + 1)).0-branch"
  ;;
point)
  _new_version="$_major.$_minor.$(($_point + 1)).0"
  _new_branch="$_major.$_minor.$(($_point + 1))-branch"
  ;;
patch)
  _new_version="$_major.$_minor.$_point.$(($_patch + 1))"
  ;;
*)
  echo "./upversion.sh (major|minor|point|patch)"
  exit 1
esac
...
if [ "$_new_branch" != "" ]; then
  echo "  git checkout -b ${_new_branch}"
  echo "  git push --set-upstream origin ${_new_branch}"
fi
