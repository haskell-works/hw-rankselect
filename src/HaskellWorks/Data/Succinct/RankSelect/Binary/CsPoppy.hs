...
data CsPoppy = CsPoppy
  ...
  , csPoppyIndex :: DVS.Vector Word64
  } deriving (Eq, Show)

makeCsPoppy :: DVS.Vector Word64 -> CsPoppy
makeCsPoppy v = CsPoppy
  { csPoppyBits  = v
  , csPoppyIndex = DVS.constructN (((DVS.length v + 7) `div` 8) + 1) gen512Index
  }
...
instance Select1 CsPoppy where
  select1 (CsPoppy v i) p = toCount q * 512 + select1 (DVS.drop (fromIntegral q * 8) v) (p - s)
    where q = binarySearch (fromIntegral p) wordAt 0 (fromIntegral $ DVS.length i - 1)
          s = Count (i !!! q)
          wordAt = (i !!!)

instance Select0 CsPoppy where
  select0 (CsPoppy v i) p = toCount q * 512 + select0 (DVS.drop (fromIntegral q * 8) v) (p - s)
    where q = binarySearch (fromIntegral p) wordAt 0 (fromIntegral $ DVS.length i - 1)
