...
module HaskellWorks.Data.Succinct.BalancedParens.RangeMinMax2.L2
  ( RangeMinMax2L2(..)
  , mkRangeMinMax2L2
  ) where
...
data RangeMinMax2L2 = RangeMinMax2L2
  { rangeMinMaxL2Base     :: !RangeMinMax2L1
  , rangeMinMaxL2Min      :: !(DVS.Vector Int16)
  , rangeMinMaxL2Max      :: !(DVS.Vector Int16)
  , rangeMinMaxL2Excess   :: !(DVS.Vector Int16)
  }

instance RangeMinMax2Level RangeMinMax2L2 where
  rmmFactor _ = 32
...
class MkRangeMinMax2L2 a where
  mkRangeMinMax2L2 :: a -> RangeMinMax2L2

instance MkRangeMinMax2L2 RangeMinMax2L1 where
  mkRangeMinMax2L2 rmmL1 = result
  ...
  {-# INLINE mkRangeMinMax2L2 #-}

instance MkRangeMinMax2L2 RangeMinMax2Simple where
  mkRangeMinMax2L2 = mkRangeMinMax2L2 . mkRangeMinMax2L1
  {-# INLINE mkRangeMinMax2L2 #-}

instance MkRangeMinMax2L2 (DVS.Vector Word64) where
  mkRangeMinMax2L2 = mkRangeMinMax2L2 . mkRangeMinMax2Simple
  {-# INLINE mkRangeMinMax2L2 #-}

instance TestBit RangeMinMax2L2 where
  (.?.) = (.?.) . rangeMinMaxL2Base
  {-# INLINE (.?.) #-}

instance Rank1 RangeMinMax2L2 where
  rank1 = rank1 . rangeMinMaxL2Base
  {-# INLINE rank1 #-}

instance Rank0 RangeMinMax2L2 where
  rank0 = rank0 . rangeMinMaxL2Base
  {-# INLINE rank0 #-}

instance BitLength RangeMinMax2L2 where
  bitLength = bitLength . rangeMinMaxL2Base
  {-# INLINE bitLength #-}

instance RangeMinMax2Derived RangeMinMax2L2 where
  type RangeMinMax2Base RangeMinMax2L2 = RangeMinMax2L1
  rmmBase = rangeMinMaxL2Base
  {-# INLINE rmmBase #-}

instance RangeMinMax2 RangeMinMax2L2 where
...
instance OpenAt RangeMinMax2L2 where
  openAt = openAt . rangeMinMaxL2Base
  {-# INLINE openAt #-}

instance CloseAt RangeMinMax2L2 where
  closeAt = closeAt . rangeMinMaxL2Base
  {-# INLINE closeAt #-}

instance NewCloseAt RangeMinMax2L2 where
  newCloseAt = newCloseAt . rangeMinMaxL2Base
  {-# INLINE newCloseAt #-}

instance FindOpenN RangeMinMax2L2 where
  findOpenN = findOpenN . rangeMinMaxL2Base
  {-# INLINE findOpenN #-}

instance FindCloseN RangeMinMax2L2 where
  findCloseN v s p = (+ 1) `fmap` rmmFindClose v (fromIntegral s) (p - 1)
  {-# INLINE findCloseN #-}

instance FindOpen RangeMinMax2L2 where
  findOpen v p = if v `openAt` p then Just p else findOpenN  v (Count 0) (p - 1)
  {-# INLINE findOpen #-}

instance FindClose RangeMinMax2L2 where
  findClose v p = if v `closeAt` p then Just p else findCloseN v (Count 1) (p + 1)
  {-# INLINE findClose #-}

instance Enclose RangeMinMax2L2 where
  enclose v = findOpenN v (Count 1)
  {-# INLINE enclose #-}

instance BalancedParens RangeMinMax2L2
